...
public class RTEventSystemRaycaster : BaseRaycaster
{
    ...
    {
        if (rtCamera == null || rtDisplay == null) return;
        if (!RemapToRT(eventData.position, out Vector2 rtPos)) return;
        ...
        RaycastWorldSpaceCanvas(ray, eventData, resultAppendList);
    }

    private void RaycastWorldSpaceCanvas(Ray ray, PointerEventData eventData, List<RaycastResult> results)
    {
        foreach (var graphic in FindObjectsByType<Graphic>(FindObjectsSortMode.None))
        {
            ...
            Plane plane = new(rt.forward, rt.position);
            ...
            if (!rt.rect.Contains(localHit)) continue;
    ...
    private bool RemapToRT(Vector2 screenPos, out Vector2 rtPos)
    {
        rtPos = Vector2.zero;
        RectTransform rect = rtDisplay.rectTransform;

        if (!RectTransformUtility.ScreenPointToLocalPointInRectangle(
            rect, screenPos, null, out Vector2 localPoint))
            return false;

        Vector2 rectSize = rect.rect.size;

        if (localPoint.x < -rectSize.x * 0.5f || localPoint.x > rectSize.x * 0.5f ||
            localPoint.y < -rectSize.y * 0.5f || localPoint.y > rectSize.y * 0.5f)
            return false;
        ...
        rtPos = new Vector2(normalized.x * rtSize.x, normalized.y * rtSize.y);
        return true;
    }
}